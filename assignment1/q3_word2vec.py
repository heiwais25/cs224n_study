...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    targetVector = np.eye(V)[target]

    softmaxVector = softmax(np.dot(outputVectors, predicted.T))
    delta = (softmaxVector - targetVector)[:,np.newaxis]

    cost = -np.log(softmaxVector[target])
    gradPred = np.sum(np.dot(delta.T, outputVectors), axis=0)
    grad = np.dot(delta, predicted[:,np.newaxis].T)
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for adjacentWord in contextWords:
        ...
        newCost, newGradPred, newGrad = word2vecCostAndGradient(
            predicted, adjacentWordIdx, outputVectors, dataset
        )
